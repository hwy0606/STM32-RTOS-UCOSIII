...
/* 第3步：串口发送DMA配置 */ 	 
    ...
    DMA_DeInit(DMA2_Stream7);  									//DMA通道配置  DMA2数据流7通道4
    DMA_InitStructure.DMA_Channel = DMA_Channel_4;   				   
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&USART1->DR); //外设地址      
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)USART1_SEND_DATA; //内存地址     
    ...
    DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single;         //指定的Burst转移配置内存传输    单次传输    
    DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_Single; //指定的Burst转移配置外围转移    单次传输
...
/* 第3步：串口接收DMA配置 */     
    ...
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&USART1->DR);  //外设地址      
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)USART1_RECEIVE_DATA;//内存地址      
    ...
    DMA_Cmd(DMA2_Stream5,ENABLE);  										 //使能通道  
} 
